...
MAIN() {
	...
	perl ~/cheng/pipelines/Queena/lib/00.Commbin/test_bin/stat_otu_tab.pl -unif min exported/feature-table.taxonomy.txt -prefix exported/Relative/otu_table --even exported/Relative/otu_table.even.txt -spestat exported/Relative/classified_stat_relative.xls
	perl ~/cheng/pipelines/Queena/lib//00.Commbin/bar_diagram.pl -table exported/Relative/classified_stat_relative.xls -style 1 -x_title "Sample Name" -y_title "Sequence Number Percent" -right -textup -rotate='-45' --y_mun 1,7 > exported/Relative/Classified_stat_relative.svg
	...
	for n3 in 1 2 3;
		do echo $n3;
		~/cheng/pipelines/Queena/lib/HC/Predict_fun/config/convert_percent.py -i feature-table.metagenome.L${n3}.txt;
		~/cheng/pipelines/Queena/MetaGenome_pipeline_V2.2/lib/04.Function/lib/KEGG/bin/get_table_head2.pl percent.feature-table.metagenome.L${n3}.txt 35 -trantab > percent.feature-table.metagenome.L${n3}.tab
		~/cheng/pipelines/Queena/MetaGenome_pipeline_V2.2/lib/00.Commbin/top10/bar_diagram.pl  -right -grid -rotate='-45' -x_title 'Sample Name' -y_title 'Relative Abundance' --y_mun 0.25,4 --height 350 -table percent.feature-table.metagenome.L${n3}.tab > percent.feature-table.metagenome.L${n3}.svg
		~/cheng/pipelines/Queena/MetaGenome_pipeline_V2.2/lib/00.Commbin/cluster/cluster.pl  -BC -Z -x percent.feature-table.metagenome.L${n3}.txt > level1.relative.tree
		~/cheng/pipelines/Queena/MetaGenome_pipeline_V2.2/lib/00.Commbin/cluster/draw_tree.pl -bun 0.25,4 -bline -type 4  level1.relative.tree  percent.feature-table.metagenome.L${n3}.tab --flank_x 100 >  tree.feature-table.metagenome.L${n3}.svg
		~/cheng/pipelines/Queena/lib/04.Diversity/lib/PCA/PCA.R.pl $PWD/percent.feature-table.metagenome.L${n3}.txt 0.2 $PWD/PCA_L${n3}
	done;	
	cd ..
...
COMMENT3
	...
	perl ~/cheng/pipelines/Queena/lib/00.Commbin/test_bin/table_data_svg.pl --colors cyan-orange R_output/bray_matrix.txt R_output/wunifrac_matrix.txt R_output/unifrac_matrix.txt --symbol 'Beta Diversity' > R_output/BetaDiversity_heatmap.svg
